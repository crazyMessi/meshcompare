	...
	:QGLWidget(),_md(md),_gpumeminfo(gpumeminfo),_perbatchtriangles(perbatchtriangles), _minfacessmoothrendering(minfacespersmoothrendering),_highprecision(highprecision),_timer(this)
{
	...
	if (!tryInitializeGL())
		throw MLException(QStringLiteral("Unable to create a valid OpenGL context for shared MeshLab rendering."));
...
{
	if (!tryInitializeGL())
		throw MLException(QStringLiteral("Unable to make the shared MeshLab OpenGL context current."));
}

bool MLSceneGLSharedDataContext::tryInitializeGL()
{
	if (!hasUsableOpenGLContext())
		return false;

	QGLContext* oldone = const_cast<QGLContext*>(QGLContext::currentContext());
	makeCurrent();
	if (QGLContext::currentContext() != context()) {
		if (oldone != NULL && QGLContext::currentContext() != oldone)
			oldone->makeCurrent();
		return false;
	}
...
{
	if (_meshboman.find(mmid) != _meshboman.end())
		return;
...
void MLSceneGLSharedDataContext::updateGPUMemInfo()
{
	...
	QGLContext* ctx = makeCurrentGLContext();
	if (QGLContext::currentContext() != context()) {
		if (ctx != NULL && QGLContext::currentContext() != ctx)
			ctx->makeCurrent();
		return;
	}
	GLint allmem = 0;
...
bool MLSceneGLSharedDataContext::hasUsableOpenGLContext() const
{
	QGLContext* glcontext = context();
	return glcontext != NULL && isValid() && glcontext->isValid();
}
