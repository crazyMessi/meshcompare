...
struct MLPerViewGLOptions : public vcg::RenderingModalityGLOptions
{
	...
	bool _peredge_slivertri_enabled;
	bool _peredge_boundaryloop_enabled;
		...
		:vcg::RenderingModalityGLOptions()
	{
		...
		_peredge_slivertri_enabled = false;
		_peredge_boundaryloop_enabled = false;
  ...
  size_t serialize(std::string& str) const
  {
    ...
    str.append(((_peredge_slivertri_enabled) ? "1" : "0"));
    str.append(((_peredge_boundaryloop_enabled) ? "1" : "0"));
  ...
  void deserialize(std::string& str)
  {
    ...
    _peredge_slivertri_enabled = bset[32];
    _peredge_boundaryloop_enabled = bset[33];
  }
...
private:
	void copyData(const  MLPerViewGLOptions& opts)
	{
		...
		_peredge_slivertri_enabled = opts._peredge_slivertri_enabled;
		_peredge_boundaryloop_enabled = opts._peredge_boundaryloop_enabled;
