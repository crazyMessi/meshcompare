...
{
	(void) minpolnumpersmoothshading;
    MLRenderingData dt;
    if (meshmodel == NULL)
        return;
    ...
    else
    {
        ...
        if (meshmodel->cm.FN() > 0)
        {
            ...
            tmpatts.reset();
            tmpatts[MLRenderingData::ATT_NAMES::ATT_VERTPOSITION] = true;
            // Prefer face normals by default so dense meshes do not become visually smoothed
            // just because they cross the automatic smooth-rendering face-count threshold.
            tmpatts[MLRenderingData::ATT_NAMES::ATT_VERTNORMAL] = false;
