...
namespace vcg
{
	...
	protected:
		...
		bool manageBuffers()
		{
			...
			_borendering = arebuffersok || correctlyallocated;
			return _borendering;
		}
	...
	private:
		...
		bool buffersMemoryManagementFunction(const InternalRendAtts& tobeallocated, const InternalRendAtts& tobedeallocated, const InternalRendAtts& tobeupdated)
		{
			...
			buffersDeAllocationRequested(tobedeallocated);

			// The memory released above is already reflected by MemoryInfo. Checking
			// only the net delta would count the released bytes twice and can make the
			// shared budget grow beyond its configured capacity.
			if (!_gpumeminfo.isAdditionalMemoryAvailable(newallocatedmem))
			{
				std::cout << "no additional memory available!!! memory required: "
					<< newallocatedmem << std::endl;
				deallocateAllBufferObjects();
				_borendering = false;
				return false;
			}
			else
			{
				...
				while ((it != _bo.end()) && (!failedallocation))
				{
					...
					if (tobeallocated[boname])
					{
						...
						failedallocation = !_gpumeminfo.isAdditionalMemoryAvailable(dim);
						if (!failedallocation)
						{
							// Clear a previous GL error so this allocation is judged only by
							// the calls below.
							...
							failedallocation = cbo->_bohandle == 0 ||
								err != GL_NO_ERROR ||
								glIsBuffer(cbo->_bohandle) != GL_TRUE;
						}
						if (!failedallocation)
						{
						...
						{
							if (cbo->_bohandle != 0)
								glDeleteBuffers(1, &cbo->_bohandle);
							cbo->_bohandle = 0;
							cbo->_size = 0;
							cbo->_isvalid = false;
							cbo->_memorytracked = false;
		...
		void buffersDeAllocationRequested(const InternalRendAtts& rq)
		{
			...
			if (deallocated)
				synchronizeBufferDeallocation();
		}

		void deallocateAllBufferObjects()
		{
			bool deallocated = false;
			for (unsigned int ii = 0; ii < INT_ATT_NAMES::enumArity(); ++ii)
			{
				GLBufferObject* bobj = _bo[ii];
				if (bobj != NULL &&
					(bobj->_bohandle != 0 || bobj->_size > 0 ||
					 bobj->_memorytracked || _currallocatedboatt[ii]))
				{
					bufferDeAllocationRequested(INT_ATT_NAMES(ii));
					deallocated = true;
				}
			}
			if (deallocated)
		...
		void synchronizeBufferDeallocation()
		{
			glFlush();
			glFinish();
		}
		...
		void bufferDeAllocationRequested(INT_ATT_NAMES att)
		{
			...
			if (bobj->_memorytracked && bobj->_size > 0)
		...
		bool isBufferReady(INT_ATT_NAMES att, bool replicated) const
		{
			const unsigned int ind = att;
			if (ind >= _bo.size())
				return false;

			const GLBufferObject* bobj = _bo[ind];
			const size_t expectedsize = boExpectedSize(att, replicated);
			return expectedsize > 0 && bobj != NULL &&
				_currallocatedboatt[att] && bobj->_isvalid &&
				bobj->_memorytracked && bobj->_bohandle != 0 &&
				bobj->_size == expectedsize;
		}

		bool areTriangleBuffersReady(const InternalRendAtts& req) const
		{
			if (!isBORenderingAvailable())
				return false;

			const bool replicated =
				InternalRendAtts::replicatedPipelineNeeded(_currallocatedboatt);
			for (unsigned int ii = INT_ATT_NAMES::ATT_VERTPOSITION;
				ii <= INT_ATT_NAMES::ATT_WEDGETEXTURE; ++ii)
			{
				const INT_ATT_NAMES att(ii);
				if (req[att] && !isBufferReady(att, replicated))
					return false;
			}

			return req[INT_ATT_NAMES::ATT_VERTPOSITION] &&
				(replicated ||
				 isBufferReady(INT_ATT_NAMES(INT_ATT_NAMES::ATT_VERTINDICES), false));
		}

		bool arePointBuffersReady(const InternalRendAtts& req) const
		{
			if (!isBORenderingAvailable())
				return false;
			...
			const bool replicated =
				InternalRendAtts::replicatedPipelineNeeded(_currallocatedboatt);
			const unsigned int pointattributes[] = {
				INT_ATT_NAMES::ATT_VERTPOSITION,
				INT_ATT_NAMES::ATT_VERTNORMAL,
				INT_ATT_NAMES::ATT_VERTCOLOR,
				INT_ATT_NAMES::ATT_VERTTEXTURE};
			for (unsigned int ii = 0;
				ii < sizeof(pointattributes) / sizeof(pointattributes[0]); ++ii)
			{
				const INT_ATT_NAMES att(pointattributes[ii]);
				if (req[att] && !isBufferReady(att, replicated))
					return false;
			}
			return req[INT_ATT_NAMES::ATT_VERTPOSITION];
		...
		bool areEdgeBuffersReady(const InternalRendAtts& req) const
		{
			if (!arePointBuffersReady(req))
				return false;
			return isBufferReady(
				INT_ATT_NAMES(INT_ATT_NAMES::ATT_EDGEINDICES),
				InternalRendAtts::replicatedPipelineNeeded(_currallocatedboatt));
		}
		...
		void drawFilledTriangles(const InternalRendAtts& req, const GL_OPTIONS_DERIVED_TYPE* glopts, const std::vector<GLuint>& textureindex = std::vector<GLuint>()) const
		{
			...
			if (areTriangleBuffersReady(req))
				drawTrianglesBO(req, textureindex);
			else
			{
				updateClientState(InternalRendAtts());
		...
		struct GLBufferObject
		{
			GLBufferObject(size_t components, GLenum gltype, GLenum clientstatetag, GLenum target)
				:_size(0),
				 ...
				 _isvalid(false),
				 _memorytracked(false),
				 _gltype(gltype),
				 _target(target),
				 _clientstatetag(clientstatetag),
				 _bohandle(0)
			{
			}
