		...
		vcg::CallBackPos*)
{
	...
	for (QDomElement node = root.firstChildElement(); !node.isNull(); node = node.nextSiblingElement()) {
		if (QString::compare(node.nodeName(), "MeshGroup") == 0) {
			QDomElement mesh;
			QString filen, label;
			mesh = node.firstChildElement("MLMesh");
			while (!mesh.isNull()) {
				//return true;
				...
				mesh = mesh.nextSiblingElement("MLMesh");
			}
		}
		...
		{
			QDomElement raster;
			raster = node.firstChildElement("MLRaster");
			while (!raster.isNull())
			{
				//return true;
				md.addNewRaster();
				...
				QDomNode sh = raster.firstChildElement();
				...
				while (!el.isNull())
				{
					...
					el = el.nextSiblingElement("Plane");
				}
				raster = raster.nextSiblingElement("MLRaster");
			}
		}
	}
